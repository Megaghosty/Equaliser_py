...
def generate_and_save_plots():
    ...
    axs[0, 0].plot(time, data, label='Original')
    axs[0, 0].plot(time, filtered_data, label='Filtré')
    axs[0, 0].set_title('Signal Original vs Filtré')
    ...
    axs[0, 1].plot(freqs[:len(freqs)//2], np.abs(fft_original)[:len(freqs)//2], label='Original')
    axs[0, 1].plot(freqs[:len(freqs)//2], np.abs(fft_filtered)[:len(freqs)//2], label='Filtré')
    ...
    for i, filtre in enumerate(filtres_numeriques):
        ...
        axs[1, 0].plot(h, label=f'Filtre {i+1}')
    axs[1, 0].set_title('Réponses Impulsionnelles')
    ...
    for i, filtre in enumerate(filtres_numeriques):
        ...
        axs[1, 1].plot(w[:len(w)//2], H[:len(H)//2], label=f'Filtre {i+1}')
    axs[1, 1].set_title('Réponses en Fréquence')
    axs[1, 1].set_xlabel('Fréquence normalisée')
